...
def train2(num=50, N0=56):
    ...
    test_x = [np.random.rand(2, 1) for _ in range(3)]
    test_xd = [np.random.rand(2, 1) for _ in range(3)]
    ...
    for i in range(num):
        ...
        X = [compute_X(xi - xdi, xdi) for xi, xdi in zip(test_x, test_xd)]
        U = [compute_u(X[i], W_actor[i])[0, 0] for i in range(3)]
        x_next, xd_next = step(U, i, test_x, test_xd, p)

        record.append(
            [W_critic, W_actor,
             test_x, test_xd, U])
        test_x, test_xd = x_next, xd_next
